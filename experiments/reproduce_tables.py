#!/usr/bin/env python3
"""
reproduce_tables.py - ProvGuard-RAG experimental result generator.
...
This script underpins a planning-draft Experiments section. After real
experimental logs become available, regenerate the same .tex tables and
.pdf figures from the real logs. The macro names in values.tex are stable
so the narrative does not need to be re-edited when numbers change.
"""
...
def main():
    ...
    with open(os.path.join(EXP_DIR, "values.tex"), "w") as f:
        ...
        f.write("% Do not edit by hand. Numbers used in the manuscript narrative come from here.\n")
        f.write("\n".join(MACROS) + "\n")
